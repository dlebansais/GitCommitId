...
internal partial class Program
{
    ...
    private int ExecuteProgram()
    {
        ...
        return IsUpdate || IsReplace
            ? UpdateFileCommitId(FileName, IsReplace)
            : IsClear
                ? ClearFileCommitId(FileName)
                : ReadFileCommitId(FileName);
    }
    ...
    private int ReadFileCommitId(string fileName)
    {
        try
        {
            ...
            {
                Output($"File doesn't contain Git info: {exception.Message}.", isError: true);
    ...
    private int GetRepositoryAddress(string folder, out string repositoryAddress)
    {
        while (folder != null)
        {
            ...
            if (File.Exists(GitPath))
            {
                try
                {
                    using FileStream GitPathStream = new(GitPath, FileMode.Open, FileAccess.Read, FileShare.Read);
                    using StreamReader GitPathReader = new(GitPathStream, Encoding.ASCII);
        ...
        while (folder != null)
        {
            ...
            if (File.Exists(GitPath))
            {
                try
                {
                    ...
                    if (Spec != null)
                    {
                        if (Spec.StartsWith("ref: ", StringComparison.Ordinal))
                            Head = Spec[5..];
        ...
        try
        {
            ...
            foreach (KeyValuePair<string, StringTable> StringEntry in StringFileInfo.Strings)
                foreach (KeyValuePair<string, StringTableEntry> Entry in StringEntry.Value.Strings)
                    ...
                    {
                        _ = StringEntry.Value.Strings.Remove(RepositoryAddressString);
                        Output("Repo removed.");
            ...
            foreach (KeyValuePair<string, StringTable> StringEntry in StringFileInfo.Strings)
                foreach (KeyValuePair<string, StringTableEntry> Entry in StringEntry.Value.Strings)
                    ...
                    {
                        _ = StringEntry.Value.Strings.Remove(CommitIdString);
